    ...
    if request.method == 'POST':
        ...
        # ketika pencet buy, lakukan enkripsi data lalu save ticket, buat qr code 
            ...
            ticketqr = Ticket(
                ...
                check_in=False,  #Initialize check_in as False
            )
            ...
            # pesan logger
            logger.debug(f"isi data tiket untuk QR Code: {ticketqr}")
            logger.debug(f"isi data tiket terenkripsi untuk database: {ticketdb}")
            
            # menyimpan tiket yang dienkripsi untuk database
            ticketdb.save()
            
            logger.debug("Tiket berhasil disimpan di database")
